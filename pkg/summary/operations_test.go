...
// TestNormalizeConditionsOperation covers the per-condition error/transitioning
// flags, which are produced by the same code path.
func TestNormalizeConditionsOperation(t *testing.T) {
	obj := makeOperationObj("EncryptionKeyRotation", "InProgress", "Preflight",
		cond("InProgress", "True", "WaitingForPlanApplied", "Waiting in step Preflight: failing plan for machine-7qrnd"),
		cond("Paused", "False", "NotPaused", ""),
		cond("Pending", "False", "InProgress", "Operation now in progress"),
	)

	NormalizeConditions(obj)
	...
	expected := []struct {
		...
		{condType: "InProgress", transitioning: true},
		{condType: "Paused"},
		{condType: "Pending"},
	}

	conditions := data.Object(obj.Object).Slice("status", "conditions")
	assert.Len(t, conditions, len(expected))
	for i, want := range expected {
		assert.Equal(t, want.condType, conditions[i].String("type"))
		assert.Equal(t, want.err, conditions[i].Bool("error"), "error on %s", want.condType)
		assert.Equal(t, want.transitioning, conditions[i].Bool("transitioning"), "transitioning on %s", want.condType)
	}
}

// TestCheckGenericTransitioningNonOperation guards against the operation
// dispatch capturing objects it should not.
func TestCheckGenericTransitioningNonOperation(t *testing.T) {
	obj := data.Object{"apiVersion": "management.cattle.io/v3", "kind": "Cluster"}
	conditions := []Condition{
		NewCondition("Pending", "False", "SomeReason", "something went wrong"),
	}

	actual := checkGenericTransitioning(obj, conditions, Summary{})

	assert.Equal(t, "pending", actual.State)
	assert.True(t, actual.Error)
	assert.Equal(t, []string{"something went wrong"}, actual.Message)
}
